...
function Deanacedemic() {
  const [students, setStudents] = useState([]);
 const [deanAcademic, setdeanAcademic] = useState('')
...
const addReview = async () => {
  try {
    await axios.post(`http://localhost:8080/api/dealer/deanAcademictask/${selectedStudentId}`, {
      deanAcademic: deanAcademic,
    });
          ...
          <div className="modal-dialog modal-dialog-centered" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Add Dean Academic Review</h5>
                <button type="button" className="btn-close" onClick={closeModal}></button>
              </div>
              <div className="modal-body">
                <div className="form-group">
                  <label>Dean. Academic Review</label>
                  <input
                    type="text"
                    className="form-control"
                    placeholder="Enter Review"
                    value={deanAcademic}
                    onChange={(e) => setdeanAcademic(e.target.value)}
                    required
                  />
                </div>
